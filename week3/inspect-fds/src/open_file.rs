...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let name = match fs::read_link(&fd_path).ok()?.to_str() {
            Some(fd_link) => Self::path_to_name(fd_link),
            None => {
                println!("not a valied link");
                return None;
            }
        };

        let path_string = fs::read_to_string(&fdinfo_path).ok()?;
        let cursor = match Self::parse_cursor(&path_string) {
            Some(cursor) => cursor,
            None => {
                println!("not a valied cursor");
                return None;
            }
        };

        let access_mode = match Self::parse_access_mode(&path_string) {
            Some(access_mode) => access_mode,
            None => {
                println!("not a valied access mode");
                return None;
            }
        };

        let open_file = Self::new(name, cursor, access_mode);
        println!("{:?}", open_file);
        Some(open_file)
